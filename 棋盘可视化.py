...
board[game_engine.pos_to_idx((8,10))]=chess_defs.Block(terrain=chess_defs.Terrain.PLAIN,
                                       piece=chess_defs.Piece(owner=chess_defs.Owner.A,
...
while running:
    ...
    for x in range(17):
        for y in range(17):
            block = board[game_engine.pos_to_idx((x,y))]
            #绘制地形
    ...
    for i in game_engine.get_valid_moves(board, (8, 10), chess_defs.Owner.A, 1):
        for x, y in i:
